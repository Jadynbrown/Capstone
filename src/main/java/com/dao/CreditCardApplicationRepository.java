...
public interface CreditCardApplicationRepository extends JpaRepository<CreditCardApplication, Integer>{
	...
	@Query(value = "select c.region as region, count(*) as countOfApplication from customer c inner join credit_card_application "
	...
	@Query(value = "select cu.employment as employment, count(*) as countOFApplication from customer cu inner join credit_card_application cca "
	...
	//10
	@Query(value = "SELECT AVG(DATEDIFF(credit_card_application_approval_date, credit_card_application_apply_date)) as averageDaysToApprove from credit_card_application WHERE credit_card_application_status = 'R'", nativeQuery = true)
	...
	//15
	@Query(value = "SELECT SUM(IF(marital_status = 'M', 1, 0)) AS countOfMarried, SUM(IF(marital_status = 'S', 1, 0)) AS countOfSingle FROM customer; ", nativeQuery = true)
	IMaritalDemographic maritalDemographics();
	
	@Query(value = "select count(*) as countOfSize, household_size as householdSize from customer group by household_size ", nativeQuery = true)
	List<IHouseholdDemographics> householdsizeDemographics();
	
	@Query(value = "select count(*) as numberOfCustomers, gender as gender from customer group by gender", nativeQuery = true)
	List<IGenderDemographic> genderDemographic();
	
	@Query(value = "select count(*) as numberOfCustomers, region as region from customer group by region", nativeQuery = true)
